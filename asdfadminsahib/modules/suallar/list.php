<?php
    ...
    $arr = $crud->list_for_categories($_GET['module'], 'fennler',"t1.id, t1.fenn_id, t1.ad, t2.ad as `fenn`",'t1.fenn_id=t2.id'," AND t1.fenn_id={$fenn}");
                ...
                <tbody>
                    ...
                    for ($i=0;$i<count($arr);$i++) {
                        ...
                        foreach ($arr[$i] as $key=>$value) {
                            ...
                            if ($key == 'fenn') {
                                $value = <<<HTML
                                    <a href="?module=fennler&action=edit&id={$arr[$i]['fenn_id']}">{$value}</a>
HTML;
                            }
                            echo "<td>".$value."</td>";
                        }
...
HTML;
            ...
            <button class="btn btn-warning logicalDeleteRows" style="outline: none; float: right;" delete_type="sual" module="<?= $_GET['module'];?>">Delete selected</button>
        </div>
    </div>
</div>
