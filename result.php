...
if (isset($_POST['finished']) && !empty($_POST['finished'])) {
    ...
    foreach ($_POST as $key=>$value) {
        ...
        if (is_array($value)) {
            $sizinCavab = '';
            $cavabSay = $main->show_list('cavablar', " AND `sual_id`={$key} AND `duzgun` = 1");
            if (count($value) == count($cavabSay)) {
                $bool = true;
                for ($i=0; $i<count($value); $i++) {
                    $check = $main->show_list('cavablar', " AND `id`={$value[$i]}");
                    if ($check[0]['duzgun'] == 0) {
                        $bool = false;
                ...
                if ($bool == true) {
                    ...
                    $inCorrectCount++;
                    $question = $main->show_list('suallar', " AND `id`={$key}");
                    $correctAns = $main->show_list('cavablar', " AND `sual_id`={$key} AND `duzgun`=1");
                    $duzgunCavab = '';
...
HTML;
            ...
            $check = $main->show_list('cavablar', " AND `id`={$value}");
...
HTML;
    ...
    echo <<<HTML
        ...
        <div class="alert alert-info" role="alert" style="display: block; margin: 20px 3%;">
            <strong style="text-transform: capitalize;">{$check_user['name']} {$check_user['surname']},</strong>
...
HTML;
    ...
    $arr['user_id'] = $_SESSION['client_id'];
